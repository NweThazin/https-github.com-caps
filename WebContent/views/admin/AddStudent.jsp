...
<div style="text-align:center;">
	...
	<form action="/Team04CAPS/StudentController" method="post"
		name="studentForm">
		<center>
			<table cellpadding=4 cellspacing=2 border=0>
				...
				<tr>
					...
					<td><c:if test="${param['insert']==t}">
							<input type="text" name="studentID" value="${studentID}" size=25
								maxlength=20 readonly="readonly" class="txtGroup"/>
							<input type="hidden" name="ins" value="true" />
						</c:if> <c:if test="${param['update']==t}">
							<input type="text" name="studentID" value="${param['studentID']}"
								size=25 maxlength=20 readonly="readonly" class="txtGroup"/>
				...
				<tr>
					...
					<td><input type="text" name="FirstName"
						value="${param['FirstName']}" size=25 maxlength=20 required class="txtGroup"/></td>
				</tr>
				<tr>
					<td><fmt:message key="student.secondName" /></td>
					<td><input type="text" name="SecondName"
						value="${param['SecondName']}" size=25 maxlength=20 required class="txtGroup"/></td>
				</tr>
				<tr>
					<td><fmt:message key="student.address" /></td>
					<td><input type="text" name="Address"
						value="${param['Address']}" size=25 maxlength=35 required class="txtGroup"/></td>
				</tr>
				<tr>
					<td><fmt:message key="student.yearStudy" /></td>
					<td><input type="text" name="YearofStudy"
						value="${param['YearofStudy']}" size=25 maxlength=20 required class="txtGroup"/></td>
				</tr>
				<tr>
					<td><fmt:message key="student.email" /></td>
					<td><input type="email" name="Email" value="${param['Email']}"
						size=25 maxlength=35 required class="txtGroup"/></td>
				</tr>
				<tr>
				...
				<tr>
					<td><c:if test="${param['insert']==t}">
							...
							<input class="buttonGroup" type="submit" value="Update Student">
							<input type="hidden" name="ins" value="false" />
						</c:if></td>
